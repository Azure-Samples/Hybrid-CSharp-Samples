        ...
        public static async Task runSample(string tenantId, string subscriptionId, string servicePrincipalId, string servicePrincipalSecret, string location, string armEndpoint)
        {
            ...
            if (resourceGroup != null)
            {
                ...
                if (storageAccount != null)
                {
                    ...
                    try
                    {
                        var storageAccountKeys = storageAccount.GetKeys().Value.Keys;
                        ...
                        var regeneratedStorageAccountKey = storageAccount.RegenerateKey(StorageAccountRegenerateKeyContent).Value.Keys;
                    ...
                    try
                    {
                        ...
                        storageAccountPatch.Encryption = new Encryption()
                        {
                            KeySource = KeySource.MicrosoftStorage,
                            Services = new EncryptionServices
                            {
                                Blob = new EncryptionService { Enabled = true, KeyType = "Service" },
                                File = new EncryptionService { Enabled = true, KeyType = "Service" }
                            }
                        };
                        ...
                        var status = storageAccountResource.Value.Data.Encryption.Services.Blob.Enabled;
